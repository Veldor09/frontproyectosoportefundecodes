...
function SetPasswordInner() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!token) { setError('Token inválido o ausente. Usa el enlace del correo.'); return; }
    ...
    try {
      ...
      if (res.data?.ok) {
        setMessage('¡Listo! Tu contraseña fue activada. Redirigiendo al login…');
        setNewPassword('');
        setConfirmPassword('');
      ...
      const backendMsg =
        err?.response?.data?.message || err?.response?.data?.error || err?.message || 'Error al guardar la contraseña.';
    ...
    <main className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2 drop-shadow-lg">Activar cuenta</h1>
          <p className="text-white/90 text-sm drop-shadow">Establece tu contraseña para acceder al sistema</p>
        </div>
